...
define(['require'],function(require) {
  ...
  const WifiKnownNetworks = {
    ...
    handleScanResult(allNetworks, callback) {
    ...
    scan(callback) {
      ...
      req.onsuccess = () => {
        ...
        const availableNetworksReq = wifiManager.getNetworks();
        availableNetworksReq.onsuccess = () => {
          const availableNetworks = availableNetworksReq.result;

          for (i = 0; i < allNetworks.length; ++i) {
            // eslint-disable-next-line
            const index = availableNetworks.findIndex(item => {
              // eslint-disable-next-line
              let securityStr = item.security[0];
              switch (item.security[0]) {
                case 'WPA-PSK':
                case 'WPA2-PSK':
                case 'WPA/WPA2-PSK':
                case 'SAE':
              ...
              return (
                `${allNetworks[i].ssid}+${allNetworks[i].security[0]}` ===
                `${item.ssid}+${securityStr}`
              );
            });

            if (index !== -1) {
              allNetworks[i].relSignalStrength =
                availableNetworks[index].relSignalStrength;
            }
          }
          this.handleScanResult(allNetworks, callback);
        };
        availableNetworksReq.onerror = error => {
          console.warn('availableNetworks Error : ', error);
          const defaultSignalStrength = 100;
          for (i = 0; i < allNetworks.length; i++) {
            allNetworks[i].relSignalStrength = defaultSignalStrength;
          }
          this.handleScanResult(allNetworks, callback);
        };
      };
