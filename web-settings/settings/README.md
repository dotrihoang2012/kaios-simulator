...
`SettingsService` provides a navigate function for panel navigation. It call to its show and hide functions when navigating (see the module/panel.js section).
...
`SettingsPanel` extends `Panel` with basic settings services. It presets the UI elements based on the values in settings db in onBeforeShow. In onInit it parses the panel element for activating links. It also removes listeners in onHide so that we can avoid unwanted UI updates when the panel is outside of the viewport.
