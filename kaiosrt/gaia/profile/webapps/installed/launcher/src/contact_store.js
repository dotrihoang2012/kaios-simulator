...
class ContactStore extends BaseEmitter {
  start() {
  ...
  findBy(filter, callback) {
    ...
    if (!filterValue.length) {
      setTimeout(() => {
        callback(
           ...
           *    error when passed to ContactManager.find, bailing out
           *    early avoids the additional overhead of making a
           *    request that we know will fail.
           *
           * 2. Missing ContactsManager API or empty but
     ...
     *     multi-word queries
     *
     * 7. Make a Contact.find request with the length predominate term
     *     as the filter.filterValue.
     *
     * 8. In the ContactManager.find request success handler, filter the
