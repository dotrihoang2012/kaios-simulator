'use strict';
/* global WebActivity, FtuLauncher, InitLogoHandler, Service, AppOrigin
   SettingsObserver */
...
(function(exports) {
  ...
  function ExternalStorageMonitor() {
    ...
    // removable.
    var storages = navigator.b2g.getDeviceStorages('sdcard');
  ...
  const INTERNAL_MEDIA_VOLUME_NAME = 'sdcard';
  const EXTERNAL_MEDIA_VOLUME_NAME = 'sdcard1';
  const SDCARD_DETECT_KEY = 'external-storage-detected';
  ...
  ExternalStorageMonitor.prototype = {
     ...
     */
    init: function() {
      window.removeEventListener('ftudone', this._init);
      window.removeEventListener('ftuskip', this._init);
      let sdcardDetect = false;
      if (!this._storage) {
        // set default media location to internal storage if needed
        this.resetDefaultMediaLocation();
        this.showStorageInfoIfNecessary(sdcardDetect);
      } else {
        this._storage.available().then((result) => {
          if (result === 'unavailable') {
            this.resetDefaultMediaLocation();
          } else { // 'shared' & 'available' and other cases
            sdcardDetect = true;
    ...
    start: function() {
      ...
      if (FtuLauncher.isFinished()) {
        if (!InitLogoHandler.finished) {
          window.addEventListener('logohidden', this._init);
        } else {
          this.init();
        }
      } else {
        window.addEventListener('ftudone', this._init);
        window.addEventListener('ftuskip', this._init);
      }
    },

    resetDefaultMediaLocation: function() {
      if (!this._internalStorage) {
        return;
      }

      Service.request('SettingsCore:get', DEFAULT_MEDIA_VOLUME_KEY).then((value) => {
        if (value !== 'sdcard') {
          SettingsObserver.setValue([{
            name: DEFAULT_MEDIA_VOLUME_KEY,
            value: INTERNAL_MEDIA_VOLUME_NAME
          }]);
        }
      });
    },

    showStorageInfoIfNecessary: function(sdcardDetect) {
      window.asyncStorage.getItem(SDCARD_DETECT_KEY, (value) => {
        let lastDetectedTime = value;
        if (!lastDetectedTime && sdcardDetect) {
          window.asyncStorage.setItem(SDCARD_DETECT_KEY, Date.now());
          SettingsObserver.setValue([{
            name: DEFAULT_MEDIA_VOLUME_KEY,
            value: EXTERNAL_MEDIA_VOLUME_NAME
          }]);

          Service.request('DialogService:hide', 'set-external-storage');
          this.showSdcardDetectDialog();
        } else if (lastDetectedTime !== '' && !sdcardDetect) {
          window.asyncStorage.setItem(SDCARD_DETECT_KEY, '');
          this.showNoSdcardNotification();
        }
      });
    },

    showSdcardDetectDialog: function() {
      const _ = window.api.l10n.get;
      Service.request('DialogService:show', {
        id: 'set-external-storage',
        header: _('storage'),
        content: _('set-sdcard-as-default-media-location'),
        translated: true,
        type: 'confirm',
        ok: 'ok',
        cancel: 'no',
    ...
    showNoSdcardNotification: function() {
      var _ = window.api.l10n.get;
      var body = _('no-sdcard-title');
      var title = _('storage');
      var notification = new Notification(title, {
        body: body,
        icon: `${AppOrigin.getOrigin('system')}/style/icons/system_84.png`,
        tag: 'external-storage-detected'
      });
      notification.addEventListener('click', () => {
        ...
        Service.request('DialogService:show', {
          id: 'no-sdcard-alert',
          header: _('storage'),
          content: _('no-sdcard-content'),
          translated: true,
          type: 'alert',
          noClose: true,
          onOk: () => {
            Service.request('DialogService:hide', 'no-sdcard-alert');
          }
        });
      });
    },
    ...
    recogniseStorageActions: function() {
      ...
      if (actionsFlags.match(this.actionRecognised)) {
        ...
        // can remove set Settings key here.
    ...
    enableStorageUnrecognised: function(enabled) {
      SettingsObserver.setValue([{
        name: 'volume.external.unrecognised',
        value: enabled
      }]).then(() => {
        var msg = '[ExternalStorageMonitor] set settings key onsuccess';
        this.debug(msg);
      }, (error) => {
        var msg = '[ExternalStorageMonitor] set settings key error:' + error;
        this.debug(msg);
      });
    },
    ...
    fireNotification: function(title, body, openSettings) {
      var iconUrl = `${AppOrigin.getOrigin('system')}/style/storage_status/notification_sd_card.png`;
