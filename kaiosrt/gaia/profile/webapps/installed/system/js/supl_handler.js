/* global BaseModule, Service, ScreenManager, SettingsObserver */
'use strict';
...
(function() {
  ...
  BaseModule.create(SuplHandler, {
    ...
    notify: function(detail) {
      ...
      notification.addEventListener('click', () => {
        ...
        Service.request('DialogService:show', {
          ...
          noClose: true,
          onOk: () => {
            Service.request('DialogService:hide', 'supl-notification-alert');
          }
        });
      });
    },
    ...
    showDialog: function(detail) {
      ...
      ScreenManager.turnScreenOn('supl-show-dialog');
    ...
    sendChoice: function(data, value) {
      ...
      // Send the choice back via Settings based on Gecko's design. Set it a
      // postive id if user allows the action or set it to be negative if not.
      SettingsObserver.setValue([{
        name: 'supl.verification.choice',
        value: data.vid * (value ? 1 : -1)
      }]);
