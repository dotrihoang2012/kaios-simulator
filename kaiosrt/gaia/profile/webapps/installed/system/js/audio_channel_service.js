/* global BaseModule, Service, AudioChannelController */
'use strict';

(function() {
  const audioChannelManager = navigator.b2g.audioChannelManager;
  const systemAudioChannels = audioChannelManager.allowedAudioChannels;
  /**
  ...
  AudioChannelService.STATES = [
    'getSystemAudioControllers'
  ];
  ...
  BaseModule.create(AudioChannelService, {
    ...
    // Store audio channels for System app
    _systemAudioControllers: null,
    ...
    _start: function() {
      ...
      this._audioChannelWeights = new Map([
        ['none', 0], ['system', 1], ['normal', 2],
        ['content', 3], ['alarm', 4], ['ringer', 5],
        ['telephony', 6], ['notification', 7], ['publicnotification', 8]
      ]);
      this._interruptedAudioChannels = [];
      this._muteSystemAudioChannels();
      this.debug('Start Audio Channel Manager');
      Service.registerState('contentChannelIsPlaying', this);
    },

    contentChannelIsPlaying : function() {
      let isPlaying = false;
      this._activeAudioChannels.forEach((audioChannel) => {
        if (!isPlaying && audioChannel.isPlaying() &&
          audioChannel._channel.name === 'content') {
          isPlaying = true;
        }
      });
      return isPlaying;
    ...
    _manageAudioChannels: function(audioChannel) {
      if (audioChannel.isActive()) {
        ...
        const musicAppURL = window.AppOrigin.getManifestURL('music');
        const fmAppURL = window.AppOrigin.getManifestURL('fm');
        const app = audioChannel.app;
        const activeAppURL = app && app.manifestUrl;
        let mutexAppURL = '';
        if (activeAppURL === musicAppURL) {
          mutexAppURL = fmAppURL;
        } else if (activeAppURL === fmAppURL) {
          mutexAppURL = musicAppURL;
        }

        this._activeAudioChannels.forEach((audioChannel) => {
          const appURL = audioChannel.app && audioChannel.app.manifestUrl;
          if (mutexAppURL && appURL === mutexAppURL &&
            audioChannel._states.playing) {
            let killEvent = document.createEvent('CustomEvent');
            killEvent.initCustomEvent('killapp', false, false, {
              manifestUrl: appURL
            });
    ...
    _fadeInFadedOutAudioChannels: function() {
      ...
      this._interruptedAudioChannels.forEach((audioChannel) => {
        audioChannel.isFadingOut() && audioChannel
          .setPolicy({ isNeededToFadeOut: false })
          .proceedPolicy();
      });
    ...
    _resumeAudioChannels: function(app) {
      if (Service.query('isShuttingDown')) {
        return;
      }
      ...
      if (app) {
        ...
        // App will be still active if it's opening an inline activity.
        // Need to stop here.
        this._activeAudioChannels.forEach((channel) => {
          if (channel.app && channel.app !== app && 'normal' === channel.name) {
            channel.setPolicy({ isAllowedToPlay: false }).proceedPolicy();
            this._activeAudioChannels.delete(channel.instanceID);
          }
        });
      }
      ...
      // Send audiochannelchanged event to SoundManager.
      var channel = this._getTopPriorityAudioChannel();
      this.publish('audiochannelchanged', { channel: channel });
    },
    ...
    _muteSystemAudioChannels: function() {
      if (this._isSystemMuted) {
        return;
      }
      if (systemAudioChannels && systemAudioChannels.length) {
        this._systemAudioControllers = new Map();
        Array.from(systemAudioChannels).forEach((audioChannel) => {
          const controller = new AudioChannelController(this, audioChannel);
          controller.setPolicy({ isAllowedToPlay: false }).proceedPolicy();
          this._systemAudioControllers.set(audioChannel.name, controller);
        });
        this._isSystemMuted = true;
        window.dispatchEvent(new CustomEvent('muteSystemAudioChannels'));
      }
    },

    getSystemAudioControllers: function() {
      return this._systemAudioControllers;
    }
  });
}());
