'use strict';
/* global MapMessage, PbmapDialog, SettingsObserver */
(function() {
  ...
  function getConfirm(evt) {
    function callSystemConfirm(text) {
      window.api.l10n.formatValue('confirmTitle').then(title => {
        let option = {
    ...
    SettingsObserver.getValue(BLUETOOTH_MAP).then((value) => {
      let addr = value;
