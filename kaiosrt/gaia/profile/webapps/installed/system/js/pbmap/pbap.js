...
/* global ContactToVcardBlob, PbapPhonebook, PbmapDialog, SettingsObserver */

(function() {
  let bluetoothManager = navigator.b2g.bluetooth;
  let adapter = null;
  let pb;
  let pairedName = '';
  let _debug = true;
  ...
  function obexPassword(evt) {
    function passwordDialog(text) {
      ...
      Promise.all(responseValues.map(key => window.api.l10n.formatValue(key)))
      .then(values => {
        ...
        PbmapDialog.show(option, (result) => {
          debug('bluetooth obex pwd: ' + result.password);
          if (result.value) {
  ...
  function getConfirm(evt) {
    ...
    SettingsObserver.getValue('bluetooth.pbap').then((value) => {
      let addr = value;

      // If the user has confirmed, don't show dialog again
      if (addr && addr === evt.address) {
        debug('User has confirmed, auto accept.');
        evt.handle.accept();
        pbapInit();
      } else {
  ...
  function saveUserSelect(address) {
