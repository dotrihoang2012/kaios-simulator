'use strict';
/* global PowerSave, ScreenManager, SettingsObserver, Service, soundManager */
...
(function(exports) {
  ...
  BatteryOverlay.prototype = {
    ...
    SOUNDFILE_LOWBATTERY: 'resources/sounds/Low battery.ogg',
    _battery: null,
    ...
    // Initialize the battery object. Note that synchronous access to _battery
    // from outside (eg. power_save.js) is now potentially racy.
    ...
    init: function bm_init() {
      ...
      SettingsObserver.observe('audio.volume.notification', 1, (value) => {
        this.silent = value === 0;
      });
    ...
    playAudio: function bm_playAudio(sound) {
      if (!this.audio) {
    ...
    displayIfNecessary: function bm_display() {
      ...
      this.playAudio(this.SOUNDFILE_LOWBATTERY);
      Service.request('SystemToaster:show', {
        text: window.api.l10n.get('battery-almost-empty-plug-in-your-charger')
      });
    },
    ...
    showBatteryFullNotification: function() {
      var title = window.api.l10n.get('battery-full-title') || '';
      var body = window.api.l10n.get('battery-full-body') || '';
      var notification = new window.Notification(title, {
        body: body,
        tag: 'batteryFull',
        icon: window.AppOrigin.getOrigin('system') +
          '/style/notifications/images/battery_full.png'
      });
