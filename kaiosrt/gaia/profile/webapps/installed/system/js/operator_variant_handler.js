/* globals ApnHelper, LazyLoader, OperatorVariantHelper, SettingsObserver,
...
(function(exports) {
  ...
  // When support cdma module, we should set the config to true,
  // and gaia will provide all the apn items to gecko,
  // otherwise, gaia just need to provide the preferred apn.
  ...
  var OPERATOR_VARIANT_FILE =
    window.AppOrigin.getOrigin('shared') + '/resources/apn.json';

  var APN_TYPES = ['default', 'mms', 'supl', 'dun', 'ims', 'ia', 'hipri',
    'xcap', 'cbs', 'fota', 'Emergency'];
  var AUTH_TYPES = ['none', 'pap', 'chap', 'papOrChap'];
  var APN_KEYS = ['authtype', 'user', 'password', 'proxy', 'port', 'mmsc',
    'mmsproxy', 'mmsport', 'protocol', 'roaming_protocol', 'bearer'];
  ...
  OperatorVariantHandler.prototype = {
        ...
        if (!persistKeyNotSet) {
          ...
          this.filterApnsByMvnoRules(0, result, [], '', '', [],
            (function onFinishCb(filteredApnList) {
              this.buildCompleteDefaultApnSettings(filteredApnList);
    ...
    filterApnsByMvnoRules: function ovh_filterApnsByMvnoRules(apnIndex,
      ...
      if (apnIndex === allApnList.length) {
        SettingsObserver.getValue('operatorvariant.mvno').then((value) => {
          var mvnoInfos = value;
          if (!mvnoInfos || !Array.isArray(mvnoInfos)) {
            mvnoInfos = [null, null];
          }
          mvnoInfos[this._iccCardIndex] = mvnoMatchedInfo;
          SettingsObserver.setValue([{
            name: 'operatorvariant.mvno',
            value: mvnoInfos
          }]);
        });
      ...
      listMvnoType = listMvnoType && listMvnoType.toLowerCase();
      var listMvnoMatchData = apn.mvno_match_data || '';
      if (listMvnoType === 'gid' || listMvnoType === 'imsi') {
        listMvnoMatchData =
          listMvnoMatchData && listMvnoMatchData.toLowerCase();
      }
      ...
      if (!listMvnoType ||
          ...
          if (!listMvnoType) {
            let existMvnoApn = filteredApnList.find((apn) => {
              return !!apn.mvno_type;
            });
            if (existMvnoApn === undefined) {
      ...
      let matchedInfo = mvnoMatchedInfo.find((matchItem) => {
        return matchItem.mvnoType === listMvnoType &&
          matchItem.mvnoMatchData === listMvnoMatchData;
      });
      if (matchedInfo) {
        ...
        const iccCard = navigator.b2g.iccManager &&
          navigator.b2g.iccManager.getIccById(this._iccId);
        ...
        request.onsuccess = (function onSuccessHandler() {
          ...
          mvnoMatchedInfo.push(matchedInfo);
        ...
        request.onerror = (function onErrorHandler() {
          console.log('operator APN: matchMvno error ' + listMvnoType + ', '
            + listMvnoMatchData);
          this.filterApnsByMvnoRules(apnIndex + 1,
                                     allApnList,
                                     filteredApnList,
    ...
    updateOperatorVariantSettings:
      function ovh_updateOperatorVariantSettings(key, value) {
        SettingsObserver.getValue(key).then((getValue) => {
          var originalSetting = getValue || ['', ''];
    ...
    convertApnSettings: function ovh_convertApns(apnSettings) {
      ...
      return apnSettings.map(function(apn) {
        ...
        APN_KEYS.forEach(function(key) {
          var empty;
          switch (key) {
            case 'authtype':
              empty = AUTH_TYPES[apnClone.authtype] || 'notDefined';
              // Convert process force update the value.
              apnClone[key] = empty;
              break;
            case 'protocol':
    ...
    buildCompleteDefaultApnSettings:
      function ovh_buildCompleteDefaultApnSettings(allApnList) {
        ...
        SettingsObserver.getValue('ril.data.default.apns').then((value) => {
          var result = value || {};
          result[this._iccCardIndex] = validApnSettings;
          SettingsObserver.setValue([{
            name: 'ril.data.default.apns',
            value: result
          }]);
        });
      },
    ...
    buildApnSettings: function ovh_buildApnSettings(allApnList) {
      ...
      // Check if enabled cdma feature or not.
      DeviceCapabilityManager.get('device.cdma-apn').then((isEnabled) => {
        if (isEnabled) {
          this.buildAllApnSettings().then((allApnSettings) => {
            this.saveDefaultApnSettings(apnSettings, allApnSettings);
          });
        } else {
          this.buildPreferredApnSettings(apnSettings);
        }
      });
    },

    resetDefaultApnSettings: function ovh_resetDefaultApnSettings(apnSettings) {
      SettingsObserver.getValue('ril.data.default.apnSettings').then(value => {
        let result = value;
        ...
        SettingsObserver.setValue([{
          name: 'ril.data.default.apnSettings',
          value: result
        }]);
      });
    },
    ...
    /**
     * Build the preferred data call settings which using for
     * building data calls.
     *
     * @param {Array} apnSettings The array of settings used for default
     *                            data call setting.
     */
    buildPreferredApnSettings:
      function ovh_buildPreferredApnSettings(apnSettings) {
        this.resetCpApns();
        const simApnSettingsKey = [
          'ril.data.dm.apnSettings.sim1',
          'ril.data.dm.apnSettings.sim2'
        ];
        const key = simApnSettingsKey[this._iccCardIndex];
        SettingsObserver.setValue([{
          name: key,
          value: apnSettings
        }, {
        ...
        this.resetDefaultApnSettings(apnSettings);
      },

    resetCpApns: function ovh_resetCpApns() {
      SettingsObserver.getValue('ril.data.cp.apns').then((value) => {
        var result = value;
        if (!result || !Array.isArray(result)) {
          result = [{}, {}];
        }
        result[this._iccCardIndex] = {};
        SettingsObserver.setValue([{
          name: 'ril.data.cp.apns',
          value: result
        }]);
      });
    },
    ...
    /**
     * Build all the data call settings for the default APN setting architecture
     *
     * @return {Array} The all matched data call settings.
     */
    buildAllApnSettings: function ovh_buildAllApnSettings() {
      return new Promise((resolve) => {
        LazyLoader.getJSON(OPERATOR_VARIANT_FILE).then((apn) => {
          var mcc = this._iccSettings.mcc;
          var mnc = this._iccSettings.mnc;
          var allApnList = ApnHelper.getAll(apn, mcc, mnc);
          var tmpApnSettings = [];
          var allApnSettings = [];

          allApnList.forEach((apn) => {
            var tmpApn = this.clone(apn);

            tmpApn.type.forEach((type) => {
              var newTmpApn = this.clone(tmpApn);

              delete newTmpApn.type;
              newTmpApn.type = [];
              newTmpApn.type.push(type);
              tmpApnSettings.push(newTmpApn);
            });
          });

          allApnSettings = this.convertApnSettings(tmpApnSettings);

          resolve(allApnSettings);
        });
      });
    },

    /**
     * Save the filtered and all data call settings to Settings.
     */
    saveDefaultApnSettings:
      function ovh_saveDefaultApnSettings(apnSettings, allApnSettings) {
        this.resetCpApns();
        const simApnSettingsKey = [
          'ril.data.dm.apnSettings.sim1',
          'ril.data.dm.apnSettings.sim2'
        ...
        SettingsObserver.getValue(key).then((value) => {
          ...
          SettingsObserver.setValue([{
            name: key,
            value: allApnSettings
          }, {
            name: 'apn.selections',
            value: null
          }]);
          this.resetDefaultApnSettings(mergedApnSettings);
        });
    },
    ...
    applyWAPUAProfileUrl: function ovh_applyWAPUAProfileUrl(uaProfile) {
      SettingsObserver.setValue([{
        name: 'wap.UAProf.url',
        value: uaProfile ? uaProfile.url : undefined
      }]);
    }
  };
