...
self.onsystemmessage = evt => {
  ...
  if (evt.name === 'activity') {
    ...
    let activityType = 'inline';
    if (handler.source.name === 'configure-window') {
      activityType = 'window';
    }
    console.log('activity activityType:', activityType);
    evt.waitUntil(
      clients
        .openWindow('/index.html#activity', { disposition: activityType })
        .then(
          client => {
