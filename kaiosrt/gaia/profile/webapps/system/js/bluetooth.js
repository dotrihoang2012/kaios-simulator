 ...
 */
/* global Service, LazyLoader, SettingsObserver, BluetoothTransfer */
/* exported Bluetooth2 */
(function(exports) {
...
Bluetooth.prototype = {
  ...
  /**
   * Hold instance of HID status changed handler.
   *
   * @private
   */
  _bindHidStatusChangedHandler: null,
  ...
  /**
   * Bluetooth PBAP/MAP profile Lazy load flag.
   *
   * @private
   */
  _isPbmapLoaded: false,
  ...
  _setProfileConnected: function bt_setProfileConnected(profile, connected) {
    this.debug('setProfileConnected: ' + profile + ' ' + connected);
  ...
  isProfileConnected: function bt_isProfileConnected(profile) {
    ...
    this.debug('isProfileConnected: ' + profile + ' ' + isConnected);
  ...
  init: function bt_start() {
    if (!window.navigator.b2g.bluetooth) {
      return;
    }

    this._bluetooth = window.navigator.b2g.bluetooth;
    ...
    this._bindHidStatusChangedHandler = function(evt) {
      this._setProfileConnected('hid', evt.status);
    }.bind(this);
  ...
  _adapterAvailableHandler: function bt__adapterAvailableHandler() {
    ...
    this._adapter.addEventListener('hidstatuschanged',
      this._bindHidStatusChangedHandler);

    if (this._isEnabled && !this.isA2DPProfileConnected) {
  ...
  _adapterUnavailableHandler: function bt__adapterUnavailableHandler() {
    ...
    this._adapter.removeEventListener('scostatuschanged',
      this._bindScoStatusChangedHandler);
    this._adapter.removeEventListener('hidstatuschanged',
      this._bindHidStatusChangedHandler);
  ...
  _requestDisableHandler: function bt__requestDisableHandler() {
    ...
    if (this._adapter) {
      if (this._adapter.state === 'disabling') {
        this.debug('bluetooth is disabling');
        return;
      }
      // the state will be dispatched in _btAdapterHandler
  ...
  /*
   * lazyLoad the PBAP/MAP profile when receive the pbap/map system message
   *
   * @private
   */
  _loadPbmapProfile: function() {
    if (!this._isPbmapLoaded) {
      LazyLoader.load([
        ...
        this.debug('bluetooth load pbap/map profile');
      }).catch(function (err) {
        console.error(err);
      });

      this._isPbmapLoaded = true;
    }
  },
  ...
  _connectPairedDecive: function bt__connectPairedDecive() {
    SettingsObserver.getValue(this._deviceConnectedKey).then((value) => {
      var address = value;
      if (address) {
  ...
  swMessageHandler: function(event) {
    ...
    switch (type) {
      ...
      case 'bluetooth-opp-transfer-complete':
        ...
        break;
      case 'bluetooth-pbap-request':
      case 'bluetooth-map-request':
        this._loadPbmapProfile();
        break;
      default:
        break;
    }
