...
(function (exports) {
  var DownloadHelper = (function () {
    ...
    SettingsObserver.observe(STORAGE_NAME_KEY, STORAGE_NAME_BY_DEFAULT,
    ...
    function getBlob(download) {
      ...
      storeAvailableReq.onsuccess = function available_onsuccess() {
    ...
    Action.prototype = {
      ...
      run: function a_run() {
        ...
        const activity = new WebActivity(this.name, this.data);
        activity.start().then(
          result => {
       ...
       */
      _onsuccess: function a_onsuccess(result) {
       ...
       */
      _onerror: function a_onerror(err) {
        this.req.failed({
          code: err
        });
      }
    ...
    function runAction(actionType, download) {
      ...
      window.setTimeout(function launching() {
        ...
        if (state === 'succeeded' || state === 'finalized') {
          LazyLoader.load([
              'js/download/download_formatter.js'
            ],
            function loaded() {
              var type = getType(download);

              //
              // The 'open' action will always launch an activity using the
              // original content type to allow for third party applications to
              // handle arbitrary types of content.
              //
    ...
    function handlerError(error, download, cb) {
      LazyLoader.load('js/download/download_ui.js', (function loaded() {
        var req;
        ...
        if (error.message &&
          (error.message.endsWith('canceled') ||
            ...
            error.message.endsWith('Cancelled'))) {
          // Since this isn't actually an error, invoke the callback with null.
          cb && cb(null);
          // and return early.
          return;
        }
    ...
    function getFreeSpace(cb) {
      var storage = navigator.b2g.getDeviceStorage(storageName);
    ...
    function getVolume(volumeName) {
      ...
      var volumes = navigator.b2g.getDeviceStorages('sdcard');
    ...
    return {
       ...
       *
       * @param{Function} This function is performed when the free memory size
       *                  has been calculated
       */
