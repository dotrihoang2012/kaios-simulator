...
/* global LazyLoader */
/* global MimeMapper, SettingsSoftkey */
...
(function (exports) {
  var DownloadUI = (function () {
    ...
    function createConfirm(type, req, downloads) {
      ...
      var message = confirmDialog.firstElementChild;
      ...
      } else if (type.name === 'unsupported_file_type' ||
    ...
    var styleSheets = [
      'style/button/buttons.css',
      'style/header/headers.css',
    ];
    ...
    function show(type, downloads, ignoreStyles) {
      ...
      window.setTimeout(function () {
        var libs = ['js/download/download_formatter.js'];
        if (!ignoreStyles) {
          libs.push.apply(libs, styleSheets);
        }

        // We have to discover the type of UI depending on state
        // when type is null
        if (type === null) {
          type = TYPES.STOPPED;
    ...
    function showActions(download) {
      ...
      window.setTimeout(function () {
        LazyLoader.load(['js/download/download_formatter.js'],
