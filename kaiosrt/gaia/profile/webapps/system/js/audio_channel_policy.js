...
(function() {
  // The results of audio channel competition.
  var PLAY = true;
  var PAUSE = false;
  /**
  ...
  BaseModule.create(AudioChannelPolicy, {
     ...
     * Active audio channels playing new.
     */
    applyPolicy: function(newAudioChannel, activeAudioChannels) {
      ...
      if (newAudioChannelName === 'normal' &&
        newAudioChannel.app.getTopMostWindow &&
        newAudioChannel.app.getTopMostWindow() !== newAudioChannel.app) {
        allowed = false;
      }
      ...
      // Once we get conflicts, we will take higher priority policy.
      // `PAUSE` is higher than `PLAY`,
      // and doing fade out is higher than not doing fade out.
      var isAllowedToPlayForNewAudioChannel = [allowed];
      ...
      if (!isAllowedToPlayForNewAudioChannel) {
        policy.isNeededToVibrate = this._isVibrateEnabled &&
          ...
          if (!policy.isNeededToVibrate || newAudioChannelName === 'alarm') {
            policy.isNeededToResumeWhenOtherEnds =
              isNeededToResumeWhenOtherEndsForNewAudioChannel;
          }
      }
    ...
    _isNeededToResumeWhenOtherEndsForNewAudioChannel:
      ...
      return ((activeChannelName === 'telephony' && newChannelName === 'content') ||
              (activeChannelName === 'telephony' && newChannelName === 'alarm'));
    }
