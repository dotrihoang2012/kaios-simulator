...
/* globals WebActivity, Service, SettingsObserver,
...
(function(exports) {
  ...
  NfcManager.prototype = {
    ...
    start: function nm_start() {
      ...
      SettingsObserver.observe('nfc.enabled', false, this._onSettingsChanged);

      this._onDebugChanged = (enabled) => { DEBUG = enabled; };
      SettingsObserver.observe('nfc.debugging.enabled', false,
    ...
    stop: function nm_stop() {
      ...
      SettingsObserver.unobserve('nfc.enabled', this._onSettingsChanged);
      SettingsObserver.unobserve('nfc.debugging.enabled', this._onDebugChanged);
    },
     ...
     * array it can either trigger P2P sharing scenario or create WebActivity
    ...
    _changeHardwareState: function nm_changeHardwareState(state) {
      ...
      switch (state) {
        ...
        case this.NFC_HW_STATE.DISABLE_DISCOVERY:
          promise = nfcdom.stopPoll('mode_screen_off');
          break;
      ...
      promise.then(() => {
        ...
        // Reset Nfc to off state when failed to connect to nfc Hal layer.
        if (e.message === "ErrorConnect") {
          this._handleNFCOnOff(false);
          return;
        }
    ...
    _fireNDEFDiscovered: function nm_fireNDEFDiscovered(records) {
      ...
      const activity = new WebActivity(options.name, options.data);
      activity.start();
    },
     ...
     * activity options object which will be used to launch WebActivity
     * @memberof NfcManager.prototype
     * @param {Object} payload - decoded payload of first record from NDEF msg
     * @returns {Object} options - object used to construct WebActivity
     */
