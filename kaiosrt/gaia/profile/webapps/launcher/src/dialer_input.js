...
import TonePlayer from './util/tone_player';
...
export default class DialerInput extends BaseComponent {
  ...
  componentDidMount() {
    ...
    SettingsObserver.observe('phone.ring.keypad', null,
      this['_observe_phone.ring.keypad']);
    Service.register('dialerFakeSoftKey', this);
  ...
  unloadHandler = () => {
    ...
    Service.unregister('dialerFakeSoftkey', this);
  }

  dialerFakeSoftKey() {
    // The softkey needs to be set to main-view during the call
    // to avoid refreshing the softkey flashing.
    const mainViewSoftKey = {
      left: 'notifications',
      center: 'icon=all-apps',
      right: 'contacts'
    };
    SoftKeyStore.register(mainViewSoftKey, this.element);
  }
  ...
  callHandleOption() {
    ...
    if (!isEmergencyCall && this.isVTSupported) {
      let options = [{
        ...
        callback: () => {
          this.element.focus();
          this.props.dial(this.telNum, { isRtt: SettingsStore.isRttAuto() });
        },
      }, {
        id: 'video-call',
        callback: () => {
          this.element.focus();
          this.props.dial(this.telNum, { isVideo: true });
        },
      }];
  ...
  handleTelNumber() {
    ...
    /**
     * The [RTT call] option will be available while
     * the RTT preference is set to manual.
     */
    if (SettingsStore.isRttManual()) {
      options.unshift({
        ...
        callback: () => {
          this.element.focus();
          this.props.dial(this.telNum, { isRtt: true });
        }
      });
    }
